...
?>
<br><br><br>
<center><h1>Please select the room number from your seleted room type(X14)</h1></center>.
<?php
 $query="SELECT * FROM roomava where category=1;";
        mysqli_query($db,$query) or die("Query Failed");
        $result=mysqli_query($db,$query);
        while($row=mysqli_fetch_array($result)){
 $room=$row['roomno'];
 echo("<h2 style='color:yellow ;'><b><em><center>$room</em></b></center></h2>");
		}
		?><br>
<center>
